...
object Recursion:
  ...
  def countChange(money: Int, coins: List[Int]): Int = {
    def countChangeRecur(mcoins: List[Int], acc: Int): Int = {
      if (acc == 0) then 1
      else if (acc < 0 || coins.isEmpty) then 0
      else countChangeRecur(money, coins, acc - coins.head) + countChangeRecur(money, coins.tail, acc)
    }

    countChangeRecur(money, coins, money)
  }
